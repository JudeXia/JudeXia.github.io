...
<body>
	...
	<h2>温室控制</h2>
	<ul>
	<li><a href="/control/light.php" title="控制温室加热器">补光灯</a></li>
	...
	<hr/>
	<h2>当前加热器状态</h2>
	<p><?php show_heat($heat); ?></p>
	<hr/>
	<h2>可执行操作</h2>
	<p style="font-weight: bold;"><?php turn_heat($heat) ?>加热器：</p>
	<form name="form" action="/savecommand.php" method="post" target="hehe" style="text-indent: 2em;">
		<input type="text" id="device" name="device" value="heat" style="display: none;" />
		<input type="text" id="action" name="action" value="<?php echo $heat; ?>" style="display: none;" />
		持续时间(关闭加热器时不必填写)：<input type="text" id="time" name="time" />

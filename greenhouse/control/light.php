...
<body>
	...
	<hr/>
	<h2>当前补光灯状态</h2>
	<p><?php show_light($light); ?></p>
	<hr/>
	<h2>可执行操作</h2>
	<p style="font-weight: bold;"><?php turn_light($light) ?>补光灯：</p>
	<form name="form" action="/savecommand.php" method="post" target="hehe" style="text-indent: 2em;">
		<input type="text" id="device" name="device" value="light" style="display: none;" />
		<input type="text" id="action" name="action" value="<?php echo $light; ?>" style="display: none;" />
		持续时间(关闭补光灯时不必填写)：<input type="text" id="time" name="time" />
